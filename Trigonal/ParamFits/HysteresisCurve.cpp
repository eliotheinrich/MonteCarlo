...
int main(int argc, char* argv[]) {
    ...
    for (int i = 0; i < resolution; i++) {
        Bm = B_max - 2.*(B_max - B_min)/resolution*abs(i - resolution/2.);
        B1 = Bm*Bhat1;
        B2 = Bm*Bhat2;
        model1->B = B1;
        model2->B = B2;
        ...
        if (i > resolution / 2) {
            model1->save_spins(foldername + "/abi" + to_string(Bm) + ".txt");
            model2->save_spins(foldername + "/ai" + to_string(Bm) + ".txt");
        } else {
            model1->save_spins(foldername + "/abd" + to_string(Bm) + ".txt");
            model2->save_spins(foldername + "/ad" + to_string(Bm) + ".txt");
        }
