...
inline int SpinModel::flat_idx(int n1, int n2, int n3, int s) const {
    return n1 + N1*(n2 + N2*(n3 + N3*s));
}

inline Eigen::Vector4i SpinModel::tensor_idx(int i) const {
    int n1 = i % N1;
    i = i / N1;
    int n2 = i % N2;
    i = i / N2;
...
inline Eigen::Vector3d SpinModel::get_magnetization() const {
